...
// 20×20 grid scaled 12× to fill 240×240 area (centered in 240×280 display)
#define GRID         20
#define CELL         12
#define CANVAS_W     (GRID * CELL)   // 240
#define CANVAS_H     (GRID * CELL)   // 240
#define OFFSET_Y     20              // center vertically in 280
...
static const char* GROUP_NAMES[GROUP_COUNT][GROUP_MAX] = {
    // Group 0 — idle / sleepy
    { "expression sleep", "idle breathe", "idle blink", NULL },
    // Group 1 — normal pace
    { "idle look around", "expression wink", NULL, NULL },
    // Group 2 — active
    { "expression surprise", "idle look around", NULL, NULL },
    // Group 3 — heavy
    { "expression surprise", "expression wink", NULL, NULL },
};
...
// Render a frame directly to the display via GFX — no canvas buffer needed.
// Each cell is a CELL×CELL rectangle drawn with fillRect.
static void render_frame(const uint8_t *cells, const uint16_t *palette) {
    // In rotated mode, the GFX library handles the coordinate mapping
    // via MADCTL, so we always draw in the logical coordinate space.
    // The splash area is centered: x=0, y=OFFSET_Y, w=CANVAS_W, h=CANVAS_H
    for (int gy = 0; gy < GRID; gy++) {
        for (int gx = 0; gx < GRID; gx++) {
            uint8_t code = cells[gy * GRID + gx];
            uint16_t color = (palette && code < SPLASH_PALETTE_SIZE) ? palette[code] : COL_EMPTY;
            int16_t x = gx * CELL;
            int16_t y = OFFSET_Y + gy * CELL;
...
void splash_show(void) {
    active = true;
    splash_pick_for_current_rate();
    // Clear LVGL's screen to black so it doesn't paint over our GFX pixels
...
void splash_hide(void) {
    active = false;
    // When switching back to LVGL, it will redraw via lv_obj_invalidate
    gfx->fillScreen(0x0000);
}
